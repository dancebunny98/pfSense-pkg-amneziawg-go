...
// Ensure obfuscation keys always exist to avoid undefined array key warnings on PHP 8.3
foreach (array('jc', 'jmin', 'jmax', 's1', 's2', 'h1', 'h2', 'h3', 'h4', 'i1', 'i2', 'i3', 'i4', 'i5') as $obfs_key) {
        if (!isset($pconfig[$obfs_key])) {
                $pconfig[$obfs_key] = '';
        }
}
...
$section = new Form_Section('AmneziaWG Obfuscation Parameters');

$section->addInput(new Form_StaticText(
        'Hint',
        'All fields below are optional. Leave blank to omit the parameter from the generated configuration (falls back to plain WireGuard-compatible behavior for that field). Both peers of a tunnel must use identical Jc/Jmin/Jmax/S1/S2/H1-H4 values to establish a handshake.'
));

$group = new Form_Group('Junk Packets (Jc / Jmin / Jmax)');
...
$group->add(new Form_Input(
        ...
        $pconfig['jc'],
        ['placeholder' => 'e.g. 4']
))->addClass('trim')->setHelp('Junk packet count.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['jmin'],
        ['placeholder' => 'e.g. 40']
))->addClass('trim')->setHelp('Junk packet minimum size.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['jmax'],
        ['placeholder' => 'e.g. 70']
))->addClass('trim')->setHelp('Junk packet maximum size.')->setWidth(2);

$section->add($group);

$group = new Form_Group('Init/Response Junk Size (S1 / S2)');
...
$group->add(new Form_Input(
        ...
        $pconfig['s1'],
        ['placeholder' => 'e.g. 0']
))->addClass('trim')->setHelp('Init packet junk size.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['s2'],
        ['placeholder' => 'e.g. 0']
))->addClass('trim')->setHelp('Response packet junk size.')->setWidth(2);

$section->add($group);

$group = new Form_Group('Magic Headers (H1 - H4)');
...
$group->add(new Form_Input(
        ...
        $pconfig['h1'],
        ['placeholder' => 'e.g. 1']
))->addClass('trim')->setHelp('Init packet magic header.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['h2'],
        ['placeholder' => 'e.g. 2']
))->addClass('trim')->setHelp('Response packet magic header.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['h3'],
        ['placeholder' => 'e.g. 3']
))->addClass('trim')->setHelp('Cookie reply magic header.')->setWidth(2);
...
$group->add(new Form_Input(
        ...
        $pconfig['h4'],
        ['placeholder' => 'e.g. 4']
))->addClass('trim')->setHelp('Transport packet magic header.')->setWidth(2);

$section->add($group);

foreach (array('i1', 'i2', 'i3', 'i4', 'i5') as $i_field) {
        $section->addInput(new Form_Input(
                $i_field,
                strtoupper($i_field),
                'text',
                $pconfig[$i_field],
                ['placeholder' => '<b 0x...> (optional)']
        ))->addClass('trim')
          ->setHelp(strtoupper($i_field) . ' special junk packet (AmneziaWG v2, awg2 protocol). Leave blank if unused.');
}
...
if (!is_awg_tunnel_assigned($pconfig['name'])) {
	...
	$section->addInput(new Form_StaticText(
		'Firewall Rules',
		"<i class='fa-solid fa-shield-alt' style='vertical-align: middle;'></i><a style='padding-left: 3px' href='/firewall_rules.php?if={$wgg['ifgroupentry']['ifname']}'>AmneziaWG Interface Group</a>"
	));
